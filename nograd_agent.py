...
class LstmAgent(Agent):
    ...
    def read(self, inpt,wb_attack=False, f_encode=True):
        if f_encode:
            inpt = self._encode(inpt, self.model.word_dict)
        if wb_attack:
            lang_hs, lang_h = self.model.read(Variable(inpt), self.lang_h, self.ctx_h)
            ...
            return inpt, prev_read_lang_hs, prev_read_lang_h, prev_words ####### There exists a problem. The input_emb includes the context information. However we want pure language    
        else:
    ...
    def write(self, bob_ends=True, wb_attack=False):
        ...
        if bob_ends:
            ...
            _, outs, self.lang_h, lang_hs = self.model.write(self.lang_h, self.ctx_h,
                100, self.args.temperature, wb_attack=wb_attack)            
        # append new hidded states to the current list of the hidden states
    ...
    def write_rl(self):
        sentence = ['take','book','you','get','rest','<eos>']
        _, outs, self.lang_h, lang_hs = self.model.write_rl(self.lang_h, self.ctx_h, sentence)        
        self.lang_hs.append(lang_hs)
        # first add the special 'YOU:' token
        self.words.append(self.model.word2var('YOU:').unsqueeze(1))
        # then append the utterance
        self.words.append(outs)
        #print(torch.cat(self.words).size()[0],torch.cat(self.lang_hs).size()[0])
        assert (torch.cat(self.words).size()[0] == torch.cat(self.lang_hs).size()[0])        
        return self._decode(outs, self.model.word_dict)

    def write_selection(self, wb_attack=False, alice=False):
        if wb_attack:
            _, outs, lang_h, lang_hs, loss1, list_lang_hs = self.model.write(self.lang_h, self.ctx_h,
    ...
    def _choose(self, lang_hs=None, words=None, sample=False):
        ...
        class_loss = 5 * (choice_logit[idx.item()] - valid_choice)-(choice_scores[valid_set[valid_idx.item()]])        
    ...
    def choose(self, wb_attack=False, bob_ends=True, bob_out=None):
        choice, _, _, class_loss, choice_logit = self._choose()
